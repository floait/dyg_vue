...
define(['jquery', 'core'], function($, core) {
  ...
  core.json = function(routes, args, callback, hasloading, ispost) {
    ...
    var op = {
      ...
      error: function() {
        core.removeLoading();
        console.log("error in /addons/sz_yi/static/js/app/core.js at line 77, this ajax is error or return data is not json");
        // core.tip.show('请求失败');
      }
    }
    if (args && ispost) {
  ...
  //获取json2
  core.json2 = function(routes, args, callback, hasloading, ispost) {
    var url = ispost ? this.getUrl(routes) : this.getUrl(routes, args);
    var op = {
      url: url,
      type: ispost ? 'post' : 'get',
  ...
  core.tip = {
    show: function(msg, position, duration) {
      if (!msg) {
        ...
        if (m == null) {core.tip.show("没有传入信息"); return false;}
        var d = 0.2;
